  ...
  case EJsonNotation::String:
  {
    if (Property->IsA<FStructProperty>())
    {
      ...
      if (StructProperty->Struct == FWebJSFunction2::StaticStruct())
      {
        ...
        return NativeFuncs::SetPropertyValue<FStructProperty, FWebJSFunction2>(Property, Outer, Data, ArrayIndex, CallbackObject);
      }
    }
  }
  break;
...
FNativeJSStructDeserializerBackend::FNativeJSStructDeserializerBackend(FNativeJSScriptingRef InScripting, FMemoryReader &Reader)
    : FJsonStructDeserializerBackend(Reader), Scripting(InScripting)
{
}
