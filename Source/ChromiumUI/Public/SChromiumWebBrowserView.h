...
class FChromiumWebBrowserViewport;
class IChromiumWebBrowserAdapter;
class IChromiumWebBrowserDialog;
class IChromiumWebBrowserPopupFeatures;
class IChromiumWebBrowserWindow;
struct FChromiumWebNavigationRequest;
enum class EChromiumWebBrowserDialogEventResponse;
enum class EChromiumWebBrowserDocumentState;
enum class EChromiumWebBrowserConsoleLogSeverity;

DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnBeforePopupDelegate, FString, FString);
DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnCreateWindowDelegate, const TWeakPtr<IChromiumWebBrowserWindow>&, const TWeakPtr<IChromiumWebBrowserPopupFeatures>&);
DECLARE_DELEGATE_RetVal_OneParam(bool, FOnCloseWindowDelegate, const TWeakPtr<IChromiumWebBrowserWindow>&);
DECLARE_DELEGATE_RetVal_OneParam(TSharedPtr<IToolTip>, FOnCreateToolTip, const FText&);

#if WITH_CEF3
typedef SViewport SChromiumWebBrowserWidget;
#else
typedef SWidget SChromiumWebBrowserWidget;
#endif

class CHROMIUMUI_API SChromiumWebBrowserView
	: public SCompoundWidget
{
public:
	DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnBeforeBrowse, const FString& /*Url*/, const FChromiumWebNavigationRequest& /*Request*/)
	DECLARE_DELEGATE_RetVal_ThreeParams(bool, FOnLoadUrl, const FString& /*Method*/, const FString& /*Url*/, FString& /* Response */)
	DECLARE_DELEGATE_RetVal_OneParam(EChromiumWebBrowserDialogEventResponse, FOnShowDialog, const TWeakPtr<IChromiumWebBrowserDialog>&);
	...
	SLATE_BEGIN_ARGS(SChromiumWebBrowserView)
	...
	{ }
		...
		SLATE_ARGUMENT(TOptional<FChromiumBrowserContextSettings>, ContextSettings)
	 ...
	 */
	void Construct(const FArguments& InArgs, const TSharedPtr<IChromiumWebBrowserWindow>& InWebBrowserWindow = nullptr);
	...
	*/
	...
	void BindAdapter(const TSharedRef<IChromiumWebBrowserAdapter>& Adapter);

	void UnbindAdapter(const TSharedRef<IChromiumWebBrowserAdapter>& Adapter);
...
private:
	...
	void HandleBrowserWindowDocumentStateChanged(EChromiumWebBrowserDocumentState NewState);
	 ...
	 */
	bool HandleBeforeNavigation(const FString& Url, const FChromiumWebNavigationRequest& Request);
	 ...
	 */
	bool HandleCreateWindow(const TWeakPtr<IChromiumWebBrowserWindow>& NewBrowserWindow, const TWeakPtr<IChromiumWebBrowserPopupFeatures>& PopupFeatures);
	 ...
	 */
	bool HandleCloseWindow(const TWeakPtr<IChromiumWebBrowserWindow>& BrowserWindow);

	/** Callback for showing dialogs to the user */
	EChromiumWebBrowserDialogEventResponse HandleShowDialog(const TWeakPtr<IChromiumWebBrowserDialog>& DialogParams);
...
private:
	...
	TSharedPtr<IChromiumWebBrowserWindow> BrowserWindow;
	...
	TSharedPtr<FChromiumWebBrowserViewport> BrowserViewport;
	/** Viewport interface for rendering popup menus. */
	TSharedPtr<FChromiumWebBrowserViewport>	MenuViewport;
	/** The implementation dependent widget that renders the browser contents. */
	TSharedPtr<SChromiumWebBrowserWidget> BrowserWidget;

	TArray<TSharedRef<IChromiumWebBrowserAdapter>> Adapters;
