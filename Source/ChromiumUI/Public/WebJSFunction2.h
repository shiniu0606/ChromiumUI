...
#include "WebJSFunction2.generated.h"

class FWebJSScripting;

struct CHROMIUMUI_API FWebJSParam
{
  ...
  template <typename T>
  FWebJSParam(const T &Value,
              typename TEnableIf<!TIsPointer<T>::Value, UStruct>::Type *InTypeInfo = T::StaticStruct())
      : Tag(PTYPE_STRUCT), StructValue(new FStructWrapper<T>(Value))
  {
  }
  template <typename T>
...
USTRUCT()
struct CHROMIUMUI_API FWebJSCallbackBase2
{
...
protected:
  FWebJSCallbackBase2(TSharedPtr<FWebJSScripting> InScripting, const FGuid &InCallbackId)
      : ScriptingPtr(InScripting), CallbackId(InCallbackId)
  {
  }
 ...
 * FWebJSFunction2 objects represent a JS function and allow calling them from native code.
 * FWebJSFunction2 objects can also be added to delegates and events using the Bind/AddLambda method.
 */
USTRUCT()
struct CHROMIUMUI_API FWebJSFunction2
    : public FWebJSCallbackBase2
{
  GENERATED_USTRUCT_BODY()
  ...
  FWebJSFunction2(TSharedPtr<FWebJSScripting> InScripting, const FGuid &InFunctionId)
      : FWebJSCallbackBase2(InScripting, InFunctionId)
  {
  }
 ...
 *  UFUNCTIONs taking a FWebJSResponse2 will get it passed in automatically when called from a web browser.
 *  Pass a result or error back by invoking Success or Failure on the object.
 *  UFunctions accepting a FWebJSResponse2 should have a void return type, as any value returned from the function will be ignored.
 ...
 */
USTRUCT()
struct CHROMIUMUI_API FWebJSResponse2
    : public FWebJSCallbackBase2
{
  GENERATED_USTRUCT_BODY()
  ...
  FWebJSResponse2(TSharedPtr<FWebJSScripting> InScripting, const FGuid &InCallbackId)
      : FWebJSCallbackBase2(InScripting, InCallbackId)
  {
  }
