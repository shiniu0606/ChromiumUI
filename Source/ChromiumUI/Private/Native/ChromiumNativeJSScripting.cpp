...
#include "ChromiumNativeJSStructSerializerBackend.h"
...
namespace ChromiumNativeFuncs
{
	...
	bool WriteJsParam(FChromiumNativeJSScriptingRef Scripting, FJsonWriterRef Writer, const KeyType& Key, FChromiumWebJSParam& Param)
	{
		switch (Param.Tag)
		{
...
FString ChromiumGetObjectPostInitScript(const FString& Name, const FString& FullyQualifiedName)
{
	return FString::Printf(TEXT("(function(){document.dispatchEvent(new CustomEvent('%s:ready', {details: %s}));})();"), *Name, *FullyQualifiedName);
}

void FChromiumNativeJSScripting::BindUObject(const FString& Name, UObject* Object, bool bIsPermanent )
{
	...
	else
	{
		...
		SetValueScript.Append(ChromiumGetObjectPostInitScript(EscapedName, FString::Printf(TEXT("window.ue['%s']"), *EscapedName)));
		ExecuteJavascript(SetValueScript);
	}
}

void FChromiumNativeJSScripting::ExecuteJavascript(const FString& Javascript)
{
	TSharedPtr<FChromiumNativeWebBrowserProxy> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
void FChromiumNativeJSScripting::UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	const FString ExposedName = GetBindingName(Name, Object);
	if (bIsPermanent)
	{
...
int32 ChromiumParseParams(const FString& ParamStr, TArray<FString>& OutArray)
{
	OutArray.Reset();
...
bool FChromiumNativeJSScripting::OnJsMessageReceived(const FString& Message)
{
	check(IsInGameThread());

	bool Result = false;
	TArray<FString> Params;
	if (ChromiumParseParams(Message, Params))
	{
		FString Command = Params[0];
		Params.RemoveAt(0, 1);

		if (Command == ChromiumNativeFuncs::ExecuteMethodCommand)
		{
...
FString FChromiumNativeJSScripting::ConvertStruct(UStruct* TypeInfo, const void* StructPtr)
{
	TArray<uint8> ReturnBuffer;
	FMemoryWriter Writer(ReturnBuffer);

	FChromiumNativeJSStructSerializerBackend ReturnBackend = FChromiumNativeJSStructSerializerBackend(SharedThis(this), Writer);
...
FString FChromiumNativeJSScripting::ConvertObject(UObject* Object)
{
...
void FChromiumNativeJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FChromiumWebJSParam Arguments[], bool bIsError)
{
	if (!IsValid())
	{
		return;
	...
	FString CallbackScript = FString::Printf(TEXT("window.ue.$.invokeCallback('%s', %s, "), *FunctionId.ToString(EGuidFormats::Digits), (bIsError) ? TEXT("true") : TEXT("false"));
	{
		...
		ChromiumNativeFuncs::FJsonWriterRef JsonWriter = TJsonWriter<>::Create(&MemoryWriter);
		JsonWriter->WriteArrayStart();
		for (int i = 0; i < ArgCount; i++)
		{
			ChromiumNativeFuncs::WriteJsParam(SharedThis(this), JsonWriter, i, Arguments[i]);
		}
...
void FChromiumNativeJSScripting::InvokeJSFunctionRaw(FGuid FunctionId, const FString& RawJSValue, bool bIsError)
{
	if (!IsValid())
	{
		return;
...
void FChromiumNativeJSScripting::InvokeJSErrorResult(FGuid FunctionId, const FString& Error)
{
	FChromiumWebJSParam Args[1] = {FChromiumWebJSParam(Error)};
	InvokeJSFunction(FunctionId, 1, Args, true);
}

bool FChromiumNativeJSScripting::HandleExecuteUObjectMethodMessage(const TArray<FString>& MessageArgs)
{
	...
	if (ParamsSize > 0)
	{
		...
		FChromiumNativeJSStructDeserializerBackend Backend = FChromiumNativeJSStructDeserializerBackend(SharedThis(this), Reader);
...
FString FChromiumNativeJSScripting::GetInitializeScript()
{
	const FString NativeScriptingInit =
		TEXT("(function() {")
			...
			TEXT("	this.sendMessage(priority, '") + ChromiumNativeFuncs::ExecuteMethodCommand + TEXT("', id, promiseID, args.callee.name, JSON.stringify(this.argsToDict(args), this.customReplacer));")
...
void FChromiumNativeJSScripting::PageLoaded()
{
	...
	for (auto& Item : PermanentUObjectsByName)
	{
		...
		Script.Append(ChromiumGetObjectPostInitScript(Name, FString::Printf(TEXT("window.ue['%s']"), *Name)));
	}

	// Append postinit for window.ue
	Script.Append(ChromiumGetObjectPostInitScript(TEXT("ue"), TEXT("window.ue")));

	bLoaded = true;
	ExecuteJavascript(Script);
}

FChromiumNativeJSScripting::FChromiumNativeJSScripting(bool bJSBindingToLoweringEnabled, TSharedRef<FChromiumNativeWebBrowserProxy> Window)
