...
FChromiumCEFWebBrowserWindowRHIHelper::~FChromiumCEFWebBrowserWindowRHIHelper()
{
}

bool FChromiumCEFWebBrowserWindowRHIHelper::BUseRHIRenderer()
{
#if WITH_ENGINE
	if (GDynamicRHI != nullptr && FChromiumCEFWebBrowserWindow::CanSupportAcceleratedPaint())
	{
...
void FChromiumCEFWebBrowserWindowRHIHelper::UpdateCachedGeometry(const FGeometry& AllottedGeometryIn)
{
	AllottedGeometry = AllottedGeometryIn;
}

TOptional<FSlateRenderTransform> FChromiumCEFWebBrowserWindowRHIHelper::GetWebBrowserRenderTransform() const
{
	return FSlateRenderTransform(Concatenate(FScale2D(1, -1), FVector2D(0, AllottedGeometry.GetLocalSize().Y)));
}

FSlateUpdatableTexture *FChromiumCEFWebBrowserWindowRHIHelper::CreateTexture(void* SharedHandle)
{
#if WITH_ENGINE
	...
	if (FAILED(Hr))
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::CreateTexture() - - ID3D11Device::QueryInterface"));
		return nullptr;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::CreateTexture() - - ID3D11Device::OpenSharedResource"));
		return nullptr;
	}
...
void FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture(void *SharedHandle, FSlateUpdatableTexture* SlateTexture, const FIntRect& DirtyIn)
{
#if WITH_ENGINE
	check(BUseRHIRenderer());
	...
	if (FAILED(Hr))
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Device::QueryInterface"), Hr);
		return;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Device::OpenSharedResource"), Hr);
		return;
	}
	...
	if (FAILED(Hr))
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - ID3D11Texture2D::IDXGIKeyedMutex"), Hr);
		return;
	}
	...
	ENQUEUE_RENDER_COMMAND(CEFAcceleratedPaint)(
		[KeyedMutex, SlateTexture, Dirty, Tex](FRHICommandList& RHICmdList)
		{
			if (KeyedMutex)
			{
				...
				{
					UE_LOG(ChromiumLogWebBrowser, Verbose, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - failed getting sync"), E_FAIL);
				}
			}
			else
			{
				UE_LOG(ChromiumLogWebBrowser, Verbose, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - missing KeyedMutex"), E_FAIL);
				return;
			}
	});
#else
	UE_LOG(LogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - missing implementation"));
#endif // PLATFORM_WINDOWS
#else
	UE_LOG(LogWebBrowser, Error, TEXT("FChromiumCEFWebBrowserWindowRHIHelper::UpdateSharedHandleTexture() - unsupported usage, RHI renderer but missing engine"));
#endif // WITH_ENGINE
}
