...
public:
	...
	*/
	...
	virtual EChromiumWebBrowserDocumentState GetDocumentLoadingState() const = 0;
...
public:
	...
	DECLARE_EVENT_OneParam(IChromiumWebBrowserWindow, FOnDocumentStateChanged, EChromiumWebBrowserDocumentState /*NewState*/);
	virtual FOnDocumentStateChanged& OnDocumentStateChanged() = 0;

	/** A delegate to allow callbacks when a browser title changes. */
	DECLARE_EVENT_OneParam(IChromiumWebBrowserWindow, FOnTitleChanged, FString /*NewTitle*/);
	virtual FOnTitleChanged& OnTitleChanged() = 0;

	/** A delegate to allow callbacks when a frame url changes. */
	DECLARE_EVENT_OneParam(IChromiumWebBrowserWindow, FOnUrlChanged, FString /*NewUrl*/);
	virtual FOnUrlChanged& OnUrlChanged() = 0;

	/** A delegate to allow callbacks when a frame url changes. */
	DECLARE_EVENT_OneParam(IChromiumWebBrowserWindow, FOnToolTip, FString /*ToolTipText*/);
	...
	DECLARE_EVENT(IChromiumWebBrowserWindow, FOnNeedsRedraw)
	...
	DECLARE_DELEGATE_FourParams(FOnConsoleMessageDelegate, const FString& /*Message*/, const FString& /*Source*/, int /*Line*/, EChromiumWebBrowserConsoleLogSeverity /*severity*/);
	...
	DECLARE_DELEGATE_RetVal_TwoParams(bool, FOnCreateWindow, const TWeakPtr<IChromiumWebBrowserWindow>& /*NewBrowserWindow*/, const TWeakPtr<IChromiumWebBrowserPopupFeatures>& /* PopupFeatures*/)
	virtual FOnCreateWindow& OnCreateWindow() = 0;

	/** A delegate that is invoked when closing created popup windows. */
	DECLARE_DELEGATE_RetVal_OneParam(bool, FOnCloseWindow, const TWeakPtr<IChromiumWebBrowserWindow>& /*BrowserWindow*/)
	virtual FOnCloseWindow& OnCloseWindow() = 0;

	/** A delegate that is invoked when the browser needs to show a popup menu. */
	DECLARE_EVENT_OneParam(IChromiumWebBrowserWindow, FOnShowPopup, const FIntRect& /*PopupSize*/)
	virtual FOnShowPopup& OnShowPopup() = 0;

	/** A delegate that is invoked when the browser no longer wants to show the popup menu. */
	DECLARE_EVENT(IChromiumWebBrowserWindow, FOnDismissPopup)
	virtual FOnDismissPopup& OnDismissPopup() = 0;

	/** A delegate that is invoked when the browser needs to show a dialog. */
	DECLARE_DELEGATE_RetVal_OneParam(EChromiumWebBrowserDialogEventResponse, FOnShowDialog, const TWeakPtr<IChromiumWebBrowserDialog>& /*DialogParams*/)
...
protected:
	...
	/** Virtual Destructor. */
	virtual ~IChromiumWebBrowserWindow() { };
