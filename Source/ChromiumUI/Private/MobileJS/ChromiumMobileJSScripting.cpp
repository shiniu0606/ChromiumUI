...
#include "ChromiumMobileJSStructDeserializerBackend.h"
...
const FString FChromiumMobileJSScripting::JSMessageTag = TEXT("/!!com.epicgames.ue4.message/");
const FString FChromiumMobileJSScripting::JSMessageHandler = TEXT("com_epicgames_ue4_message");
...
namespace
{
	...
	const FString ScriptingInit =
		TEXT("(function() {")
			...
			TEXT("{")
#if PLATFORM_IOS
			TEXT("	window.webkit.messageHandlers.") + FChromiumMobileJSScripting::JSMessageHandler + TEXT(".postMessage(Array.prototype.map.call(arguments,function(e){return encodeURIComponent(e)}).join('/'));")
#else
			TEXT("	var req=new XMLHttpRequest();")
			TEXT("	req.open('GET', '") + FChromiumMobileJSScripting::JSMessageTag + TEXT("' + Array.prototype.map.call(arguments,function(e){return encodeURIComponent(e)}).join('/'), true);")
			TEXT("	req.send(null);")
#endif
			TEXT("}, ")
	...
	bool WriteJsParam(FChromiumMobileJSScriptingRef Scripting, FJsonWriterRef Writer, const KeyType& Key, FChromiumWebJSParam& Param)
	{
		switch (Param.Tag)
		{
...
void FChromiumMobileJSScripting::AddPermanentBind(const FString& Name, UObject* Object)
{
...
void FChromiumMobileJSScripting::RemovePermanentBind(const FString& Name, UObject* Object)
{
	const FString ExposedName = GetBindingName(Name, Object);
...
void FChromiumMobileJSScripting::BindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	TSharedPtr<IChromiumWebBrowserWindow> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
void FChromiumMobileJSScripting::UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	TSharedPtr<IChromiumWebBrowserWindow> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
void FChromiumMobileJSScripting::BindUObject(TSharedRef<class IChromiumWebBrowserWindow> InWindow, const FString& Name, UObject* Object, bool bIsPermanent)
{
	WindowPtr = InWindow;
...
void FChromiumMobileJSScripting::UnbindUObject(TSharedRef<class IChromiumWebBrowserWindow> InWindow, const FString& Name, UObject* Object, bool bIsPermanent)
{
	WindowPtr = InWindow;
...
bool FChromiumMobileJSScripting::OnJsMessageReceived(const FString& Command, const TArray<FString>& Params, const FString& Origin)
{
	bool Result = false;
...
FString FChromiumMobileJSScripting::ConvertStruct(UStruct* TypeInfo, const void* StructPtr)
{
	return TEXT("undefined");
}

FString FChromiumMobileJSScripting::ConvertObject(UObject* Object)
{
	...
	for (TFieldIterator<UFunction> FunctionIt(Class, EFieldIteratorFlags::IncludeSuper); FunctionIt; ++FunctionIt)
	{
		...
		for ( TFieldIterator<FProperty> It(Function); It; ++It )
		{
			...
			if (Param->PropertyFlags & CPF_Parm && ! (Param->PropertyFlags & CPF_ReturnParm) )
			{
				...
				if (!StructProperty || !StructProperty->Struct->IsChildOf(FChromiumWebJSResponse::StaticStruct()))
				{
					if(!firstArg)
					{
						Result.Append(TEXT(", "));
...
void FChromiumMobileJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FChromiumWebJSParam Arguments[], bool bIsError)
{
	TSharedPtr<IChromiumWebBrowserWindow> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
void FChromiumMobileJSScripting::InvokeJSFunctionRaw(FGuid FunctionId, const FString& RawJSValue, bool bIsError)
{
	TSharedPtr<IChromiumWebBrowserWindow> Window = WindowPtr.Pin();
	if (Window.IsValid())
	{
...
void FChromiumMobileJSScripting::InvokeJSErrorResult(FGuid FunctionId, const FString& Error)
{
	FChromiumWebJSParam Args[1] = {FChromiumWebJSParam(Error)};
	InvokeJSFunction(FunctionId, 1, Args, true);
}

bool FChromiumMobileJSScripting::HandleExecuteUObjectMethodMessage(const TArray<FString>& MessageArgs)
{
	...
	if (ParamsSize > 0)
	{
		...
		FChromiumMobileJSStructDeserializerBackend Backend = FChromiumMobileJSStructDeserializerBackend(SharedThis(this), MessageArgs[3]);
		FStructDeserializer::Deserialize(Params.GetData(), *Function, Backend);
	}

	if (PromiseParam)
	{
		FChromiumWebJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FChromiumWebJSResponse>(Params.GetData());
		if (PromisePtr)
		{
			*PromisePtr = FChromiumWebJSResponse(SharedThis(this), ResultCallbackId);
		}
	}
	...
	if ( ! PromiseParam ) // If PromiseParam is set, we assume that the UFunction will ensure it is called with the result
	{
		if ( ReturnParam )
		{
			...
			FChromiumMobileJSStructSerializerBackend ReturnBackend = FChromiumMobileJSStructSerializerBackend(SharedThis(this));
...
void FChromiumMobileJSScripting::InitializeScript(TSharedRef<class IChromiumWebBrowserWindow> InWindow)
{
	WindowPtr = InWindow;
...
void FChromiumMobileJSScripting::PageLoaded(TSharedRef<class IChromiumWebBrowserWindow> InWindow)
{
	WindowPtr = InWindow;
...
FChromiumMobileJSScripting::FChromiumMobileJSScripting(bool bJSBindingToLoweringEnabled)
	: FChromiumWebJSScripting(bJSBindingToLoweringEnabled)
{
}

void FChromiumMobileJSScripting::SetWindow(TSharedRef<class IChromiumWebBrowserWindow> InWindow)
{
	WindowPtr = InWindow;
}
