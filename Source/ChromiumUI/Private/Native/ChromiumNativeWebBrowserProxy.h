...
class FChromiumNativeWebBrowserProxy
	...
	, public TSharedFromThis<FChromiumNativeWebBrowserProxy>
{
	// For creating instances of this class
	friend class FChromiumWebBrowserSingleton;

private:
	FChromiumNativeWebBrowserProxy(bool bJSBindingToLoweringEnabled);
	void Initialize();
...
public:
	virtual ~FChromiumNativeWebBrowserProxy();

public:
	// IChromiumWebBrowserWindow Interface
	...
	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnDocumentStateChanged, FOnDocumentStateChanged);
	virtual FOnDocumentStateChanged& OnDocumentStateChanged() override
	{
		return DocumentStateChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnTitleChanged, FOnTitleChanged);
	virtual FOnTitleChanged& OnTitleChanged() override
	{
		return TitleChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnUrlChanged, FOnUrlChanged);
	virtual FOnUrlChanged& OnUrlChanged() override
	{
		return UrlChangedEvent;
	}

	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnToolTip, FOnToolTip);
	virtual FOnToolTip& OnToolTip() override
	{
		return ToolTipEvent;
	}

	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnNeedsRedraw, FOnNeedsRedraw);
	...
	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnShowPopup, FOnShowPopup);
	virtual FOnShowPopup& OnShowPopup() override
	{
		return ShowPopupEvent;
	}

	DECLARE_DERIVED_EVENT(FNativeWebBrowserProxy, IChromiumWebBrowserWindow::FOnDismissPopup, FOnDismissPopup);
...
private:
	...
	FChromiumNativeJSScriptingPtr Scripting;
};
