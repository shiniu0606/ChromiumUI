...
FString FNativeJSScripting::GetInitializeScript()
{
  const FString NativeScriptingInit =
      ...
      TEXT("	this.sendMessage(priority, '") +
      NativeFuncs::ExecuteMethodCommand + TEXT("', id, promiseID, args.callee.name, JSON.stringify(this.argsToDict(args), this.customReplacer));")

      // Return the promise object to the caller
      TEXT("	return promise;") TEXT("}") TEXT("},{callbacks: {value:{}}});")
...
FNativeJSScripting::FNativeJSScripting(bool bJSBindingToLoweringEnabled, TSharedRef<FNativeWebBrowserProxy> Window)
    : FWebJSScripting(bJSBindingToLoweringEnabled), bLoaded(false)
{
  WindowPtr = Window;
}
