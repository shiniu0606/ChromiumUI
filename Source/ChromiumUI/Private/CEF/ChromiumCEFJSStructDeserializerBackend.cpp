...
#include "CEF/ChromiumCEFJSStructDeserializerBackend.h"
#if WITH_CEF3
...
namespace {
	...
	void AssignTokenFromContainer(ContainerType Container, KeyType Key,  EStructDeserializerBackendTokens& OutToken, FString& PropertyName, TSharedPtr<IChromiumCefContainerWalker>& Retval)
	{
		switch (Container->GetType(Key))
		{
			case VTYPE_NULL:
			...
			case VTYPE_DICTIONARY:
			{
				...
				{
					TSharedPtr<IChromiumCefContainerWalker> NewWalker(new FChromiumCefDictionaryValueWalker(Retval, Dictionary));
			...
			case VTYPE_LIST:
			{
				TSharedPtr<IChromiumCefContainerWalker> NewWalker(new FChromiumCefListValueWalker(Retval, Container->GetList(Key)));
				Retval = NewWalker->GetNextToken(OutToken, PropertyName);
				break;
			}
			case VTYPE_BINARY:
	...
	template<typename ContainerType, typename KeyType>
	bool ReadJSFunctionProperty(TSharedPtr<FChromiumCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex, CefRefPtr<ContainerType> Container, KeyType Key )
	{
		...
		if ( !StructProperty || StructProperty->Struct != FChromiumWebJSFunction::StaticStruct())
		{
			return false;
		}
	...
	template<typename ContainerType, typename KeyType>
	bool ReadProperty(TSharedPtr<FChromiumCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex, CefRefPtr<ContainerType> Container, KeyType Key )
	{
...
TSharedPtr<IChromiumCefContainerWalker> FChromiumCefListValueWalker::GetNextToken(EStructDeserializerBackendTokens& OutToken, FString& PropertyName)
{
	TSharedPtr<IChromiumCefContainerWalker> Retval = SharedThis(this);
	Index++;
	if (Index == -1)
	{
...
bool FChromiumCefListValueWalker::ReadProperty(TSharedPtr<FChromiumCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
	return ::ReadProperty(Scripting, Property, Outer, Data, ArrayIndex, List, Index);
}

TSharedPtr<IChromiumCefContainerWalker> FChromiumCefDictionaryValueWalker::GetNextToken(EStructDeserializerBackendTokens& OutToken, FString& PropertyName)
{
	TSharedPtr<IChromiumCefContainerWalker> Retval = SharedThis(this);
	Index++;
	if (Index == -1)
...
bool FChromiumCefDictionaryValueWalker::ReadProperty(TSharedPtr<FChromiumCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
...
const FString& FChromiumCEFJSStructDeserializerBackend::GetCurrentPropertyName() const
{
	return CurrentPropertyName;
}


FString FChromiumCEFJSStructDeserializerBackend::GetDebugString() const
{
	return CurrentPropertyName;
}


const FString& FChromiumCEFJSStructDeserializerBackend::GetLastErrorMessage() const
{
	return CurrentPropertyName;
}


bool FChromiumCEFJSStructDeserializerBackend::GetNextToken( EStructDeserializerBackendTokens& OutToken )
{
	if (Walker.IsValid())
	{
...
bool FChromiumCEFJSStructDeserializerBackend::ReadProperty( FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex )
{
	return Walker->ReadProperty(Scripting, Property, Outer, Data, ArrayIndex);
}


void FChromiumCEFJSStructDeserializerBackend::SkipArray()
{
...
void FChromiumCEFJSStructDeserializerBackend::SkipStructure()
{
	EStructDeserializerBackendTokens Token;
	int32 depth = 1;
	while (GetNextToken(Token) && depth > 0)
