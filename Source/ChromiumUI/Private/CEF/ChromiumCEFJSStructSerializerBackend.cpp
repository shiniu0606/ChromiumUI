...
#include "CEF/ChromiumCEFJSStructSerializerBackend.h"
#if WITH_CEF3
...
static FString GetBindingName(const TSharedPtr<FChromiumCEFJSScripting>& Scripting, const FProperty* ValueProperty)
{
	//@todo samz - HACK
...
void FChromiumCEFJSStructSerializerBackend::AddNull(const FStructSerializerState& State)
{
...
void FChromiumCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, bool Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FChromiumCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, int32 Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FChromiumCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, double Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FChromiumCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, FString Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FChromiumCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, UObject* Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FChromiumCEFJSStructSerializerBackend::BeginArray(const FStructSerializerState& State)
{
...
void FChromiumCEFJSStructSerializerBackend::BeginStructure(const FStructSerializerState& State)
{
...
void FChromiumCEFJSStructSerializerBackend::EndArray(const FStructSerializerState& /*State*/)
{
...
void FChromiumCEFJSStructSerializerBackend::EndStructure(const FStructSerializerState& /*State*/)
{
	StackItem Previous = Stack.Pop();
...
void FChromiumCEFJSStructSerializerBackend::WriteComment(const FString& Comment)
{
	// Cef values do not support comments
}


void FChromiumCEFJSStructSerializerBackend::WriteProperty(const FStructSerializerState& State, int32 ArrayIndex)
{
	// booleans
	...
	{
		GLog->Logf(ELogVerbosity::Warning, TEXT("FChromiumCEFJSStructSerializerBackend: Property %s cannot be serialized, because its type (%s) is not supported"), *State.ValueProperty->GetName(), *State.ValueType->GetName());
	}
}
