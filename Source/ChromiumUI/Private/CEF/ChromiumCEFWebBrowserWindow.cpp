...
namespace {
	...
	class FChromiumWebBrowserClosureVisitor
		: public CefStringVisitor
	{
	public:
		FChromiumWebBrowserClosureVisitor(TFunction<void (const FString&)> InClosure)
			: Closure(InClosure)
		{ }
	...
	private:
		...
		IMPLEMENT_REFCOUNTING(FChromiumWebBrowserClosureVisitor);
	};
}
...
{
public:
	FChromiumBrowserBufferedVideo(uint32 NumFrames) 
		: FrameWriteIndex(0)
		, FrameReadIndex(0)
	...
	*/
		...
		FIntRect Dirty)
	{
		...
		FChromiumFrame& Frame = Frames[FrameWriteIndex];
	...
	FSlateTextureData* GetNextFrameTextureData()
	{
		...
		if ( FrameCount > 0 )
		{
			...
			FChromiumFrame& Frame = Frames[FrameReadIndex];
...
FChromiumCEFWebBrowserWindow::FChromiumCEFWebBrowserWindow(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FChromiumCEFBrowserHandler> InHandler, FString InUrl, TOptional<FString> InContentsToLoad, bool bInShowErrorMessage, bool bInThumbMouseButtonNavigation, bool bInUseTransparency, bool bInJSBindingToLoweringEnabled, bool bInUsingAcceleratedPaint)
...
#endif
	, Scripting(new FChromiumCEFJSScripting(InBrowser, bInJSBindingToLoweringEnabled))
#if !PLATFORM_LINUX
	, Ime(new FChromiumCEFImeHandler(InBrowser))
#endif
	, RHIRenderHelper(nullptr)
{
	...
	BufferedVideo = TUniquePtr<FChromiumBrowserBufferedVideo>(new FChromiumBrowserBufferedVideo(4));
#endif
}

void FChromiumCEFWebBrowserWindow::ReleaseTextures()
{
	for (int I = 0; I < 2; ++I)
	{
...
bool FChromiumCEFWebBrowserWindow::CreateInitialTextures()
{
	if (FSlateApplication::IsInitialized())
	{
		if (FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer())
		{
			...
			{
				if (FChromiumCEFWebBrowserWindowRHIHelper::BUseRHIRenderer() && RHIRenderHelper == nullptr)
				{
					RHIRenderHelper = new FChromiumCEFWebBrowserWindowRHIHelper;
				}
...
FChromiumCEFWebBrowserWindow::~FChromiumCEFWebBrowserWindow()
{
...
void FChromiumCEFWebBrowserWindow::LoadURL(FString NewURL)
{
	RequestNavigationInternal(NewURL, FString());
}

void FChromiumCEFWebBrowserWindow::LoadString(FString Contents, FString DummyURL)
{
	RequestNavigationInternal(DummyURL, Contents);
}

TSharedRef<SViewport> FChromiumCEFWebBrowserWindow::CreateWidget()
{
	TSharedRef<SViewport> BrowserWidgetRef =
		...
		.RenderTransform(this, &FChromiumCEFWebBrowserWindow::GetWebBrowserRenderTransform);
...
TOptional<FSlateRenderTransform> FChromiumCEFWebBrowserWindow::GetWebBrowserRenderTransform() const
{
...
void FChromiumCEFWebBrowserWindow::SetViewportSize(FIntPoint WindowSize, FIntPoint WindowPos)
{
...
FSlateShaderResource* FChromiumCEFWebBrowserWindow::GetTexture(bool bIsPopup)
{
...
bool FChromiumCEFWebBrowserWindow::IsValid() const
{
	return InternalCefBrowser.get() != nullptr;
}

bool FChromiumCEFWebBrowserWindow::IsInitialized() const
{
	return bIsInitialized;
}

bool FChromiumCEFWebBrowserWindow::IsClosing() const
{
	return bIsClosing;
}

EChromiumWebBrowserDocumentState FChromiumCEFWebBrowserWindow::GetDocumentLoadingState() const
{
	return DocumentState;
}

FString FChromiumCEFWebBrowserWindow::GetTitle() const
{
	return Title;
}

FString FChromiumCEFWebBrowserWindow::GetUrl() const
{
...
void FChromiumCEFWebBrowserWindow::GetSource(TFunction<void (const FString&)> Callback) const
{
	if (IsValid())
	{
		InternalCefBrowser->GetMainFrame()->GetSource(new FChromiumWebBrowserClosureVisitor(Callback));
	}
	else
	{
		Callback(FString());
...
void FChromiumCEFWebBrowserWindow::PopulateCefKeyEvent(const FKeyEvent& InKeyEvent, CefKeyEvent& OutKeyEvent)
{
#if PLATFORM_MAC
...
bool FChromiumCEFWebBrowserWindow::OnKeyDown(const FKeyEvent& InKeyEvent)
{
...
bool FChromiumCEFWebBrowserWindow::OnKeyUp(const FKeyEvent& InKeyEvent)
{
...
bool FChromiumCEFWebBrowserWindow::OnKeyChar(const FCharacterEvent& InCharacterEvent)
{
...
FModifierKeysState FChromiumCEFWebBrowserWindow::SlateModifiersFromCefModifiers(const CefKeyEvent& CefEvent)
{
...
bool FChromiumCEFWebBrowserWindow::OnUnhandledKeyEvent(const CefKeyEvent& CefEvent)
{
...
bool FChromiumCEFWebBrowserWindow::OnJSDialog(CefJSDialogHandler::JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	bool Retval = false;
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IChromiumWebBrowserDialog> Dialog(new FChromiumCEFWebBrowserDialog(DialogType, MessageText, DefaultPromptText, Callback));
		EChromiumWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IChromiumWebBrowserDialog>(Dialog));
...
bool FChromiumCEFWebBrowserWindow::OnBeforeUnloadDialog(const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	bool Retval = false;
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IChromiumWebBrowserDialog> Dialog(new FChromiumCEFWebBrowserDialog(MessageText, IsReload, Callback));
		EChromiumWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IChromiumWebBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EChromiumWebBrowserDialogEventResponse::Handled:
...
void FChromiumCEFWebBrowserWindow::OnResetDialogState()
{
	OnDismissAllDialogs().ExecuteIfBound();
}

void FChromiumCEFWebBrowserWindow::OnRenderProcessTerminated(CefRequestHandler::TerminationStatus Status)
{
...
FReply FChromiumCEFWebBrowserWindow::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
...
FReply FChromiumCEFWebBrowserWindow::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	FReply Reply = FReply::Unhandled();
	if (IsValid())
	{
...
FReply FChromiumCEFWebBrowserWindow::OnMouseButtonDoubleClick(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	FReply Reply = FReply::Unhandled();
	if (IsValid())
	{
...
FReply FChromiumCEFWebBrowserWindow::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	FReply Reply = FReply::Unhandled();
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::OnMouseLeave(const FPointerEvent& MouseEvent)
{
...
void FChromiumCEFWebBrowserWindow::SetSupportsMouseWheel(bool bValue)
{
	bSupportsMouseWheel = bValue;
}

bool FChromiumCEFWebBrowserWindow::GetSupportsMouseWheel() const
{
	return bSupportsMouseWheel;
}

FReply FChromiumCEFWebBrowserWindow::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	FReply Reply = FReply::Unhandled();
...
void FChromiumCEFWebBrowserWindow::OnFocus(bool SetFocus, bool bIsPopup)
{
	if (bIsPopup)
	{
...
void FChromiumCEFWebBrowserWindow::OnCaptureLost()
{
	if (IsValid())
	{
...
bool FChromiumCEFWebBrowserWindow::CanGoBack() const
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::GoBack()
{
	if (IsValid())
	{
...
bool FChromiumCEFWebBrowserWindow::CanGoForward() const
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::GoForward()
{
	if (IsValid())
	{
...
bool FChromiumCEFWebBrowserWindow::IsLoading() const
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::Reload()
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::StopLoad()
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::ExecuteJavascript(const FString& Script)
{
	if (IsValid())
	{
...
void FChromiumCEFWebBrowserWindow::CloseBrowser(bool bForce)
{
	if (IsValid())
	{
...
CefRefPtr<CefBrowser> FChromiumCEFWebBrowserWindow::GetCefBrowser()
{
	return InternalCefBrowser;
}

void FChromiumCEFWebBrowserWindow::SetTitle(const CefString& InTitle)
{
	Title = WCHAR_TO_TCHAR(InTitle.ToWString().c_str());
	TitleChangedEvent.Broadcast(Title);
}

void FChromiumCEFWebBrowserWindow::SetUrl(const CefString& Url)
{
	CurrentUrl = WCHAR_TO_TCHAR(Url.ToWString().c_str());
	OnUrlChanged().Broadcast(CurrentUrl);
}

void FChromiumCEFWebBrowserWindow::SetToolTip(const CefString& CefToolTip)
{
...
void FChromiumCEFWebBrowserWindow::GetViewRect(CefRect& Rect)
{
...
int FChromiumCEFWebBrowserWindow::GetLoadError()
{
	return ErrorCode;
}

void FChromiumCEFWebBrowserWindow::NotifyDocumentError(
...
void FChromiumCEFWebBrowserWindow::NotifyDocumentError(int InErrorCode)
{
...
void FChromiumCEFWebBrowserWindow::NotifyDocumentLoadingStateChange(bool IsLoading)
{
	if (! IsLoading)
	{
		bIsInitialized = true;
...
FSlateRenderer* const FChromiumCEFWebBrowserWindow::GetRenderer()
{
	if (FSlateApplication::IsInitialized())
	{
		if (FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer())
...
void FChromiumCEFWebBrowserWindow::HandleRenderingError()
{
	// GetRenderer handles errors already
	GetRenderer();
}

void FChromiumCEFWebBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	bool bNeedsRedraw = false;
	if (bUsingAcceleratedPaint)
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("Accelerated CEF rendering selected but OnPaint called. Disabling accelerated rendering for this browser window."));
...
void FChromiumCEFWebBrowserWindow::OnAcceleratedPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, void* SharedHandle)
{
	bool bNeedsRedraw = false;
	if (!bUsingAcceleratedPaint)
	{
		UE_LOG(ChromiumLogWebBrowser, Error, TEXT("Accelerated CEF rendering NOT selected but OnAcceleratedPaint called. Enabling accelerated rendering for this browser window."));
...
void FChromiumCEFWebBrowserWindow::UpdateVideoBuffering()
{
...
void FChromiumCEFWebBrowserWindow::OnCursorChange(CefCursorHandle CefCursor, CefRenderHandler::CursorType Type, const CefCursorInfo& CustomCursorInfo)
{
	switch (Type) {
...
bool FChromiumCEFWebBrowserWindow::OnBeforeBrowse( CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefRefPtr<CefRequest> Request, bool user_gesture, bool bIsRedirect )
{
...
FString ChromiumURLRequestSTatusToString(const CefResourceRequestHandler::URLRequestStatus& Status)
{
...
void FChromiumCEFWebBrowserWindow::HandleOnBeforeResourceLoad(const CefString& URL, CefRequest::ResourceType Type, FRequestHeaders& AdditionalHeaders)
{
	BeforeResourceLoadDelegate.ExecuteIfBound(WCHAR_TO_TCHAR(URL.ToWString().c_str()), ChromiumResourceTypeToString(Type), AdditionalHeaders);
}

void FChromiumCEFWebBrowserWindow::HandleOnResourceLoadComplete(const CefString& URL, CefRequest::ResourceType Type, CefResourceRequestHandler::URLRequestStatus Status, int64 ContentLength)
{
	ResourceLoadCompleteDelegate.ExecuteIfBound(WCHAR_TO_TCHAR(URL.ToWString().c_str()), ChromiumResourceTypeToString(Type), ChromiumURLRequestSTatusToString(Status), ContentLength);
}

EChromiumWebBrowserConsoleLogSeverity CefLogSeverityToWebBrowser(cef_log_severity_t Level)
{
	switch (Level)
	{
	case LOGSEVERITY_VERBOSE:
		return EChromiumWebBrowserConsoleLogSeverity::Verbose;
		//case LOGSEVERITY_DEBUG: // same as LOGSEVERITY_VERBOSE
		//	return Verbose;
	case LOGSEVERITY_INFO:
		return EChromiumWebBrowserConsoleLogSeverity::Info;
	case LOGSEVERITY_WARNING:
		return EChromiumWebBrowserConsoleLogSeverity::Warning;
	case LOGSEVERITY_ERROR:
		return EChromiumWebBrowserConsoleLogSeverity::Error;
	case LOGSEVERITY_FATAL:
		return EChromiumWebBrowserConsoleLogSeverity::Fatal;
	case LOGSEVERITY_DEFAULT:
	default:
		return EChromiumWebBrowserConsoleLogSeverity::Default;
	}
}

void FChromiumCEFWebBrowserWindow::HandleOnConsoleMessage(CefRefPtr<CefBrowser> Browser, cef_log_severity_t Level, const CefString& Message, const CefString& Source, int Line)
{
...
TOptional<FString> FChromiumCEFWebBrowserWindow::GetResourceContent( CefRefPtr< CefFrame > Frame, CefRefPtr< CefRequest > Request)
{
...
int32 FChromiumCEFWebBrowserWindow::GetCefKeyboardModifiers(const FKeyEvent& KeyEvent)
{
...
int32 FChromiumCEFWebBrowserWindow::GetCefMouseModifiers(const FPointerEvent& InMouseEvent)
{
...
CefMouseEvent FChromiumCEFWebBrowserWindow::GetCefMouseEvent(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, bool bIsPopup)
{
	CefMouseEvent Event;
...
int32 FChromiumCEFWebBrowserWindow::GetCefInputModifiers(const FInputEvent& InputEvent)
{
	int32 Modifiers = 0;
...
bool FChromiumCEFWebBrowserWindow::CanSupportAcceleratedPaint()
{
...
void FChromiumCEFWebBrowserWindow::UpdateCachedGeometry(const FGeometry& AllottedGeometry)
{
#if !PLATFORM_LINUX
...
void FChromiumCEFWebBrowserWindow::CheckTickActivity()
{
...
void FChromiumCEFWebBrowserWindow::RequestNavigationInternal(FString Url, FString Contents)
{
	if (!IsValid())
	{
		return;
...
bool FChromiumCEFWebBrowserWindow::HasPendingNavigation()
{
	return !PendingLoadUrl.IsEmpty();
}

void FChromiumCEFWebBrowserWindow::ProcessPendingNavigation()
{
...
void FChromiumCEFWebBrowserWindow::SetIsHidden(bool bValue)
{
	if( bIsHidden == bValue )
	{
		return;
...
void FChromiumCEFWebBrowserWindow::SetIsDisabled(bool bValue)
{
...
TSharedPtr<SWindow> FChromiumCEFWebBrowserWindow::GetParentWindow() const
{
	TSharedPtr<SWindow> ParentWindowPtr = ParentWindow.Pin();
	return ParentWindowPtr;
}

void FChromiumCEFWebBrowserWindow::SetParentWindow(TSharedPtr<SWindow> Window)
{
	ParentWindow = Window;
#if PLATFORM_WINDOWS
	if (IsValid())
...
CefRefPtr<CefDictionaryValue> FChromiumCEFWebBrowserWindow::GetProcessInfo()
{
...
bool FChromiumCEFWebBrowserWindow::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> frame, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
...
void FChromiumCEFWebBrowserWindow::BindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	Scripting->BindUObject(Name, Object, bIsPermanent);
}

void FChromiumCEFWebBrowserWindow::UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	Scripting->UnbindUObject(Name, Object, bIsPermanent);
}

void FChromiumCEFWebBrowserWindow::BindInputMethodSystem(ITextInputMethodSystem* TextInputMethodSystem)
{
#if !PLATFORM_LINUX
...
void FChromiumCEFWebBrowserWindow::UnbindInputMethodSystem()
{
#if !PLATFORM_LINUX
...
void FChromiumCEFWebBrowserWindow::OnBrowserClosing()
{
	bIsClosing = true;
}

void FChromiumCEFWebBrowserWindow::OnBrowserClosed()
{
	if(OnCloseWindow().IsBound())
	{
		OnCloseWindow().Execute(TWeakPtr<IChromiumWebBrowserWindow>(SharedThis(this)));
	}
...
void FChromiumCEFWebBrowserWindow::SetPopupMenuPosition(CefRect CefPopupSize)
{
...
void FChromiumCEFWebBrowserWindow:: ShowPopupMenu(bool bShow)
{
	if (bShow)
	{
...
void FChromiumCEFWebBrowserWindow::OnImeCompositionRangeChanged(
...
void FChromiumCEFWebBrowserWindow::UpdateDragRegions(const TArray<FChromiumWebBrowserDragRegion>& Regions)
{
	DragRegions = Regions;
}

bool FChromiumCEFWebBrowserWindow::IsInDragRegion(const FIntPoint& Point)
{
