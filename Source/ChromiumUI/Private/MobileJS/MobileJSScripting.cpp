...
namespace
{
  ...
  const FString ScriptingInit =
      ...
      TEXT("sendMessage: function()")
          TEXT("{")
#if PLATFORM_IOS
              TEXT("	window.webkit.messageHandlers.") +
      FMobileJSScripting::JSMessageHandler + TEXT(".postMessage(Array.prototype.map.call(arguments,function(e){return encodeURIComponent(e)}).join('/'));")
#else
              TEXT("	var req=new XMLHttpRequest();")
                  TEXT("	req.open('GET', '") +
      FMobileJSScripting::JSMessageTag + TEXT("' + Array.prototype.map.call(arguments,function(e){return encodeURIComponent(e)}).join('/'), true);") TEXT("	req.send(null);")
#endif
                                                 TEXT("}, ")
      ...
      TEXT("executeMethod: function(id, args)") TEXT("{") TEXT("	var self = this;") // the closures need access to the outer this object
      ...
      TEXT("	var functionJSON = Function.prototype.toJSON;") TEXT("	Function.prototype.toJSON = function(){ return self.registerCallback(this) };")

      // Create a promise object to return back to the caller and create a callback function to handle the response
      TEXT("	var promiseID;") TEXT("	var promise = new Promise(function (accept, reject) ") TEXT("	{") TEXT("		promiseID = self.registerPromise(accept, reject, args.callee.name)") TEXT("	});")

      // Actually invoke the method by sending a message to the host app
      TEXT("	this.sendMessage('") +
      ExecuteMethodCommand + TEXT("', id, promiseID, args.callee.name, JSON.stringify(this.argsToDict(args)));")

      // Restore Function.toJSON back to its old value (usually undefined) and return the promise object to the caller
      TEXT("	Function.prototype.toJSON = functionJSON;") TEXT("	return promise;") TEXT("}") TEXT("},{callbacks: {value:{}}});")

      // Create the global window.ue variable
      TEXT("window.ue = Object.create({}, {'$': {writable: false, configurable:false, enumerable: false, value:util}});") TEXT("})();");
  ...
  for (TFieldIterator<UFunction> FunctionIt(Class, EFieldIteratorFlags::IncludeSuper); FunctionIt; ++FunctionIt)
  {
    ...
    for (TFieldIterator<FProperty> It(Function); It; ++It)
    {
      ...
      if (Param->PropertyFlags & CPF_Parm && !(Param->PropertyFlags & CPF_ReturnParm))
      {
        ...
        if (!StructProperty || !StructProperty->Struct->IsChildOf(FWebJSResponse2::StaticStruct()))
        {
          if (!firstArg)
          {
            Result.Append(TEXT(", "));
...
bool FMobileJSScripting::HandleExecuteUObjectMethodMessage(const TArray<FString> &MessageArgs)
{
  ...
  if (PromiseParam)
  {
    FWebJSResponse2 *PromisePtr = PromiseParam->ContainerPtrToValuePtr<FWebJSResponse2>(Params.GetData());
    if (PromisePtr)
    {
      *PromisePtr = FWebJSResponse2(SharedThis(this), ResultCallbackId);
    }
  }
